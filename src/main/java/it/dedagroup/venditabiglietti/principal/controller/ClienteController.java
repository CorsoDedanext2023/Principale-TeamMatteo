...
@ApiResponses({
    ...
    @Operation(summary = "Endpoint che permette ad un cliente di acquistare un biglietto",
    					description = "Questo metodo serve ad acquistare un biglietto. Passando un id che rappresenta il prezzo del biglietto di un evento in un determinato settore i un controlliamo che sia valido,e passando il token dell'utente controlliamo che sia autenticato e che abbia i permessi."+
    										  "Se l'utente non ha i permessi risponderemo con codice 403, se l'id passato non è valido risponderemo 400, se non esiste un biglietto per quell'evento in quel settore risponderemo 404, altrimenti se tutto va a buon fine risponderemo 200")
    @ApiResponses({
    	...
    	@ApiResponse(responseCode = "FORBIDDEN(403)",description = "Impossibile acquistare biglietto, l'utente che vuole acquistare il biglietto non ha i permessi per farlo",content = @Content(mediaType = MediaType.ALL_VALUE)),
